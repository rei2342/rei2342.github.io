...
SYSTEM = "あなたはSNSアフィリエイトのコンテンツ戦略専門家です。JSONのみで回答してください。"
...
def analyze(case_report: dict, llm: LLMClient) -> dict:
    ...
    lp = case_report.get("lp_analysis", {}) or {}
    ...
    case_summary = (
        ...
        f"CV条件: {basic.get('conversion_condition', '不明')}\n"
        f"SNS許可: {basic.get('sns_allowed', '不明')}"
    )

    analysis_summary = (
        f"ターゲット: {lp.get('target_audience', '')}\n"
        ...
        f"市場動向: {market.get('trend', '-')} / 競合: {market.get('competition_level', '-')}"
    )

    template = load_prompt("creator_fit_prompt.md")
    prompt = (
        template
        .replace("{creator_profile}", json.dumps(profile, ensure_ascii=False, indent=2))
        .replace("{case_summary}", case_summary)
        .replace("{analysis_summary}", analysis_summary)
    )

    result = llm.call_json(prompt, SYSTEM)
    return result.get("creator_fit", {})

...
>
> 作成 2026-08-11 06:34:17 JST

記事ID：
slug：study-abroad-budget-decision-criteria
記事タイトル：留学費用を総額で見て諦めかけた社会人が、やめた3つと決めた1つの基準
画像内の短い文言：留学費用で / 諦めるその前に
補助文言：総額から見方を変える
避ける要素：年齢／TOEICスコア／金額／学習期間／利用期間／「成功」「伸びた」「改善した」／
サービスのロゴ・UI・使ったと誤認させる画面／旧サイト名／旧人物設定／
本文にない数字や結論／読めない細かい文字／文字だけのカード型
推奨ファイル名：eyecatch-study-abroad-budget-decision-criteria.png
alt：留学費用を総額ではなく週単位で考え直すことで、実現の道を探る社会人のための費用検討ガイド
カテゴリ：
使用するブランド配色：淡い桜色・クリーム・白を基調／差し色 soft sakura pink（#F2A8C0）
人物の配置：右側（腰から上）。左半分を文言のために空ける
記事固有の小物・場面：A person sitting at a desk with a notebook and pen, looking thoughtful while reviewing financial documents. A calculator and coffee cup on the side, suggesting careful budgeting and planning.
...
A person sitting at a desk with a notebook and pen, looking thoughtful while reviewing financial documents. A calculator and coffee cup on the side, suggesting careful budgeting and planning.

Japanese text to render in the left half:
- main lines (large): 留学費用で / 諦めるその前に
- supporting line (smaller, below): 総額から見方を変える

...
>
> 作成 2026-08-09 15:11:21 JST

記事ID：1001
slug：ai-era-english-how-far
記事タイトル：AI時代に必要な英語力はどこまでか｜場面から決める
画像内の短い文言：どこまでやれば / 足りるのか
補助文言：自分の場面から決める
避ける要素：年齢／TOEICスコア／金額／学習期間／利用期間／「成功」「伸びた」「改善した」／
サービスのロゴ・UI・使ったと誤認させる画面／旧サイト名／旧人物設定／
本文にない数字や結論／読めない細かい文字／文字だけのカード型
推奨ファイル名：eyecatch-ai-era-english-how-far.png
alt：読む・聞く・話す・書くの4つから、自分に必要な場面を1つ選んでいる場面
カテゴリ：AI英語学習
使用するブランド配色：淡い桜色・クリーム・白を基調／差し色 soft periwinkle blue（#A9B8E8）
人物の配置：右側（腰から上）。左半分を文言のために空ける
記事固有の小物・場面：Sakura sits cross-legged on the floor with four small hand-drawn cards laid out in front of her in a row. Each card carries only a simple icon: an open book, an ear, a speech bubble, and a pen. Her hand is reaching to pick up one card. The chosen card has a soft accent-colour glow. No words on the cards.
...
Sakura sits cross-legged on the floor with four small hand-drawn cards laid out in front of her in a row. Each card carries only a simple icon: an open book, an ear, a speech bubble, and a pen. Her hand is reaching to pick up one card. The chosen card has a soft accent-colour glow. No words on the cards.

Japanese text to render in the left half:
- main lines (large): どこまでやれば / 足りるのか
- supporting line (smaller, below): 自分の場面から決める

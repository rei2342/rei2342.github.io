...
>
> 作成 2026-08-09 15:11:19 JST

記事ID：999
slug：ai-pronunciation-app-how-to-choose
記事タイトル：AI発音矯正アプリの選び方｜自動判定でできることと、できないこと
画像内の短い文言：発音の点数が出た。 / でも直し方は？
補助文言：選ぶときの4つの確認
避ける要素：年齢／TOEICスコア／金額／学習期間／利用期間／「成功」「伸びた」「改善した」／
サービスのロゴ・UI・使ったと誤認させる画面／旧サイト名／旧人物設定／
本文にない数字や結論／読めない細かい文字／文字だけのカード型
推奨ファイル名：eyecatch-ai-pronunciation-app-how-to-choose.png
alt：発音の自動判定でできることと、点数だけでは決まらない直し方を分けて考えている場面
カテゴリ：AI英語学習
使用するブランド配色：淡い桜色・クリーム・白を基調／差し色 soft periwinkle blue（#A9B8E8）
人物の配置：右側（腰から上）。左半分を文言のために空ける
記事固有の小物・場面：Sakura wears simple over-ear headphones and holds a phone, her other hand lightly touching her jaw as if checking how she is producing a sound. Beside her floats a soft sound-wave line in the accent colour with a small empty circle at its centre, suggesting a score with nothing written in it. Absolutely no digits anywhere.
...
Sakura wears simple over-ear headphones and holds a phone, her other hand lightly touching her jaw as if checking how she is producing a sound. Beside her floats a soft sound-wave line in the accent colour with a small empty circle at its centre, suggesting a score with nothing written in it. Absolutely no digits anywhere.

Japanese text to render in the left half:
- main lines (large): 発音の点数が出た。 / でも直し方は？
- supporting line (smaller, below): 選ぶときの4つの確認

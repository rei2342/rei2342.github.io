...
>
> 作成 2026-08-14 06:33:13 JST

記事ID：
slug：working-holiday-age-limit-strategy
記事タイトル：ワーホリは30歳まで、で安心する前に。27歳から見る申請の残り回数
画像内の短い文言：残り時間は / 「年数」ではなく「回数」
補助文言：ワーホリ申請の現実的なチャンス
避ける要素：年齢／TOEICスコア／金額／学習期間／利用期間／「成功」「伸びた」「改善した」／
サービスのロゴ・UI・使ったと誤認させる画面／旧サイト名／旧人物設定／
本文にない数字や結論／読めない細かい文字／文字だけのカード型
推奨ファイル名：eyecatch-working-holiday-age-limit-strategy.png
alt：ワーキングホリデーの年齢制限を年数ではなく申請回数で数え直す記事のイメージ図
カテゴリ：
使用するブランド配色：淡い桜色・クリーム・白を基調／差し色 soft sakura pink（#F2A8C0）
人物の配置：右側（腰から上）。左半分を文言のために空ける
記事固有の小物・場面：A calendar page being flipped with a pen circling specific dates. A notepad showing a numbered checklist. The scene suggests counting down application deadlines rather than just passing years.
...
- category accent colour for this article: soft sakura pink (#F2A8C0)

Scene for this specific article:
A calendar page being flipped with a pen circling specific dates. A notepad showing a numbered checklist. The scene suggests counting down application deadlines rather than just passing years.

Japanese text to render in the left half:
- main lines (large): 残り時間は / 「年数」ではなく「回数」
- supporting line (smaller, below): ワーホリ申請の現実的なチャンス

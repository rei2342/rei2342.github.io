...
PATTERNS = [
    ...
    ("旧CTA見出し", re.compile(r"さくらが確かめた|すべて無料|▶\s*次の一手")),
]
...
def classify(sent, context):
    ...
    # 日本語は主語を落とす。「27歳。」「27歳になった。」は文だけ見ると
    # 誰の話か分からないが、前後が一人称ならさくら本人の設定。
    # **explicit な「私は27歳」より弱い推定**なので、種類を分けて出す。
    if FIRST_PERSON.search(context) and not system_ctx:
        return ("個人の設定（主語省略）", "削除する",
                "前後が一人称。主語が落ちているだけで本人の設定と読める")
    if system_ctx:
        return ("判定保留", "人が読む",
...
def main():
    ...
    KINDS = ["個人の設定", "個人の設定（主語省略）", "制度の条件（不足あり）",
             "判定保留", "制度の条件", "読者の条件", "旧サイト名", "旧CTA見出し"]
    ...
    HOW = {
        ...
        "個人の設定（主語省略）":
            "**削除する。** 前後が一人称。explicitな「私は27歳」より弱い推定なので分けてある",

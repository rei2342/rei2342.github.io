...
#
# ⚠️ 2026-08-09のDRY RUNで、既存カテゴリが7件あり中身も妥当だと分かった
# （英語学習法／英会話サービス比較／英語コーチング／海外留学・ワーホリ／
#   フィリピン・セブ留学／留学エージェント・費用）。
# 以前の監査で「未分類が主体」と書いたのは誤り。カテゴリ一覧ページが404
# だったのを見て判断してしまった。
#
# 提案していた5カテゴリのうち3つは既存と重複する。全部作ると分散するので、
# **足りない2つだけ**を新規作成し、残りは既存を使う。
NEW_CATS = [
    ("TOEIC・スコア", "toeic-score",
     "止まったスコアの内訳を分解して、次に直す場所を決める"),
    ("AI英語学習", "ai-english",
     "AI翻訳・ChatGPT・AI英会話・文字起こしをどう使うか"),
]
...
EXISTING_MAP = [
    ...
    ("留学・ワーホリ", "海外留学・ワーホリ",
     "既存を使う。フィリピン・セブ留学と留学エージェント・費用は下位として残す"),
]
...
def main():
    ...
    L.append("### 既存を使うもの（作り直さない）\n\n"
             "| 提案していた名前 | 実際に使う既存カテゴリ | 扱い |\n|---|---|---|\n")
    for a, b, c in EXISTING_MAP:
        L.append(f"| {a} | **{b}** | {c} |\n")
    L.append("\n### 新規に作るもの（既存に無いテーマだけ）\n\n")
    L.append("| 名前 | slug | 状態 |\n|---|---|---|\n")

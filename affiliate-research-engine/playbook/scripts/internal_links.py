...
ROLE = {
    ...
    # AI英語学習クラスター。親（ai2）は予約中なので、公開後に繋ぎ直す
    "993": ("child", "学習法", "ChatGPTで英語学習を始める手順"),
    ...
    "647": ("child", "英会話", "カランメソッドを試す前に確認する3つのこと"),
...
FLOW = {
    ...
    "138": ["278", "64"],
    "151": ["278", "241"],
    # 19はカニバリ候補。親ハブ278を先に置いても残るよう3本にする
    "241": ["278", "19", "151"],
    # 費用
    "23":  ["149", "131", "37"],
    "149": ["23", "131"],
...
KIND = {
    ...
    "288": "service", "647": "compare", "993": "criteria",
...
def main():
    ...
    for pid in sorted(FLOW, key=int):
        ...
        # **親ハブへ戻す1本は必ず置く。**
        # 地の文に4本あって一覧を省くと、そのクラスタのハブへ戻れなくなる
        # （2026-08-09に7本が戻り先を失っていた）。ハブだけは先頭に固定して、
        # 4本の上限より優先する
        hub = next((p for p in ROLE
                    if ROLE[p][0] == "hub" and ROLE[p][1] == cluster
                    and p != pid and p in ok), None)
        need_hub = bool(hub) and hub not in already
        if need_hub:
            cand = [hub] + [c for c in cand if c != hub]
